...
const TuesdayNotCompleted = () => {
    ...
    socket.emit('room', 'tuesday-not-completed')
    ...
    ////////////////////////////// SOCKET //////////////////////////////
    const sendUpdate = () => {
        socket.emit('send_update', {message: 'Successful update', room: 'tuesday-not-completed'});
        setFetchData(true)
    }
    ...
    const moveUpOnDeck = (indexToBeMovedUp, indexInfo) => {
        ...
        for (let i = indexToBeMovedUp - 1 ; i >= 0; i-- ) {
            if ((jobs[i].status !== 'completed' && jobs[i].status !== 'completed-late') && jobs[i].dayOfTheWeek === 2) {
        ...
        <div className="d-grid gap-2 mt-5 mb-5">
            ...
            { jobs ?
                <>
                    { changeJob === null ?
                        jobs.map((job, index) => (
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 2) ?
                            ...
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 2) ?
                                changeJob === index ?
                                    <Row className='display-row-lp-hopper-items' key={job._id}>
                                        ...
                                        <Col className='icons-in-line-right'>
                                            <GiSave onClick={() => saveJob(job._id)} size="20" color="green" className="ms-5 modal-icon"/>
...
export default TuesdayNotCompleted