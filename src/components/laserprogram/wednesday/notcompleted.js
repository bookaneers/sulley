...
const WednesdayNotCompleted = () => {
    ...
    socket.emit('room', 'wednesday-not-completed')
    ...
    ////////////////////////////// SOCKET //////////////////////////////
    const sendUpdate = () => {
        socket.emit('send_update', {message: 'Successful update', room: 'wednesday-not-completed'});
        setFetchData(true)
    }
    ...
    const moveUpOnDeck = (indexToBeMovedUp, indexInfo) => {
        ...
        for (let i = indexToBeMovedUp - 1 ; i >= 0; i-- ) {
            if ((jobs[i].status !== 'completed' && jobs[i].status !== 'completed-late') && jobs[i].dayOfTheWeek === 3) {
        ...
        <div className="d-grid gap-2 mt-5 mb-5">
            ...
            { jobs ?
                <>
                    { changeJob === null ?
                        jobs.map((job, index) => (
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 3) ?
                    ...
                    :
                        jobs.map((job, index)=>(
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 3) ?
                                ...
                                :
                                    <Row className='display-row-lp-hopper-items text-left' key={job._id}>
                                        <Col className='icons-in-line-left'>
                                            <RiArrowUpCircleFill size="25" className="ms-5 ms-5 modal-icon"  opacity="0.0" disabled/>
                                            <RiArrowDownCircleFill size="25" className="ms-5 ms-5 modal-icon"  opacity="0.0" disabled/>
                                        </Col>
...
export default WednesdayNotCompleted