...
const MondayNotCompleted = () => {
    ...
    socket.emit('room', 'monday-not-completed')
    ...
    ////////////////////////////// SOCKET //////////////////////////////
    const sendUpdate = () => {
        socket.emit('send_update', {message: 'Successful update', room: 'monday-completed'});
        setFetchData(true)
    }
    ...
    const moveUpOnDeck = (indexToBeMovedUp, indexInfo) => {
        ...
        for (let i = indexToBeMovedUp - 1 ; i >= 0; i-- ) {
            if ((jobs[i].status !== 'completed' && jobs[i].status !== 'completed-late') && jobs[i].dayOfTheWeek === 1) {
        ...
        <div className="d-grid gap-2 mt-5 mb-5">
            ...
            { jobs ?
                <>
                    { changeJob === null ?
                        jobs.map((job, index) => (
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 1) ?
...
export default MondayNotCompleted