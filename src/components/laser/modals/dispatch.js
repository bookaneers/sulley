...
const LaserDispatch = (props) => {
    ...
    const sendUpdate1 = (oneduler) => {
        socket.emit('send_update', {message: 'Successful update', room: 'laser'});
    }
    ...
    // handeles confirm, pops up robot screen if in-process
    const handleConfirm = () => {
        if (props.item.status === "wip") {
    ...
    return (
        <>
            {props.item.status === 'queue' ?
                <GiPull onClick={handleShow} className='modal-icon' size='35' />
                :
                <GiPush onClick={handleShow} className='modal-icon' size='35' />
            }
