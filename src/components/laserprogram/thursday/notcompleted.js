...
const ThursdayNotCompleted = () => {
    ...
    socket.emit('room', 'thursday-not-completed')
    ...
    ////////////////////////////// SOCKET //////////////////////////////
    const sendUpdate = () => {
        socket.emit('send_update', {message: 'Successful update', room: 'thursday-not-completed'});
        setFetchData(true)
    }
    ...
    const moveUpOnDeck = (indexToBeMovedUp, indexInfo) => {
        ...
        for (let i = indexToBeMovedUp - 1 ; i >= 0; i-- ) {
            if ((jobs[i].status !== 'completed' && jobs[i].status !== 'completed-late') && jobs[i].dayOfTheWeek === 4) {
        ...
        <div className="d-grid gap-2 mt-5 mb-5">
            ...
            { jobs ?
                <>
                    { changeJob === null ?
                        jobs.map((job, index) => (
                            ((job.status !== 'completed' && job.status !== 'completed-late') && job.dayOfTheWeek === 4) ?
                                <Row className='display-row-lp-hopper-items text-center' key={job._id}>
                                    ...
                                    <Col className='icons-in-line-right'>
                                        <GiSave size="20" className="ms-5 modal-icon" opacity="0.0" disabled/>
...
export default ThursdayNotCompleted